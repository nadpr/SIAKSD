...
class Simple_login {
        ...
        */
        public function login($username, $password) {
                ...
                //$query = $this->CI->db->get_where('karyawan',array('username'=>$username,'password' => md5($password)));

                if($query->num_rows() == 1) {
                        //ambil data user berdasar username
                        $row   = $this->CI->db->query('SELECT karyawan_id FROM karyawan where username = "'.$username.'"');
                        $admin         = $row->row();
                        $id     = $admin->karyawan_id;
